...
router.post("/simple-rating", async (req, res) => {
  try {
    const { userId, orderId, productId, rating, content } = req.body;
...
router.get("/ratable-orders/:userId", async (req, res) => {
  try {
    ...
    const completedOrders = await HoaDon.hoadon
      .find({
        userId,
    ...
    const processedOrders = await Promise.all(
      completedOrders.map(async (order) => {
        const orderProducts = await Promise.all(
          order.sanpham.map(async (product) => {
            // Kiểm tra đã đánh giá chưa
            ...
            // Lấy thông tin sản phẩm
            const productInfo = await ChitietSp.chitietsp.findById(
              product.idsp
            );
            ...
            if (product.dungluong) {
              dungluongInfo = await DungLuong.dungluong.findById(
                product.dungluong
              );
            }
  ...
  try {
    ...
    const productRatings = await Promise.all(
      products.map(async (product) => {
        ...
        if (ratings.length > 0) {
          const productTotalStars = ratings.reduce(
            (sum, r) => sum + r.rating,
            0
          );
