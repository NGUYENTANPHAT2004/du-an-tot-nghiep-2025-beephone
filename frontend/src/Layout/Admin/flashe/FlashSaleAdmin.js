...
const FlashSaleAdmin = () => {
  ...
  const fetchFlashSales = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:3005/admin/flash-sales?status=${filter}&page=${page}&limit=10`
      );
  ...
  const handleViewStats = async (id) => {
    try {
      setLoading(true);
      const response = await axios.get(
        `http://localhost:3005/admin/flash-sales/${id}`
      );
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      ...
      const response = await axios.patch(
        `http://localhost:3005/admin/flash-sales/${id}/status`,
        {
                ...
                <tr key={flashSale._id}>
                  ...
                  <td className="time-cell">
                    <div className="time-info">
                      <div className="time-range">
                        ...
                        <span>
                          {moment(flashSale.startTime).format(
                            "DD/MM/YYYY HH:mm"
                          )}
                        </span>
                        <span> - </span>
                        <span>
                          {moment(flashSale.endTime).format("DD/MM/YYYY HH:mm")}
                        </span>
                      </div>
                  ...
                  <td className="products-cell">
                    <div className="products-info">
                      <div className="total-products">
                        {flashSale.totalProducts} sản phẩm
                      </div>
                      <div className="products-qty">
                        Đã bán: {flashSale.soldQuantity}/
                        {flashSale.totalQuantity}
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="actions-cell">
                    ...
                    <button
                      ...
                      onClick={() =>
                        handleToggleStatus(flashSale._id, flashSale.isActive)
                      }
                      title={flashSale.isActive ? "Vô hiệu hóa" : "Kích hoạt"}
                    >
                      <FontAwesomeIcon
                        icon={flashSale.isActive ? faToggleOn : faToggleOff}
                      />
                    </button>
                    <button
