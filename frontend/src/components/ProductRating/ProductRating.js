...
const ProductRating = ({ productId, size = "medium", showCount = true }) => {
    ...
    const fetchRatings = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `http://localhost:3005/order-rating/product/${productId}`
        );
  ...
  if (isLoading) {
    return (
      <div
        className="product-rating-placeholder"
        style={{ height: getStarSize() }}
      ></div>
    );
  }
  ...
  if (error) {
    return (
      <div className="product-rating-error" style={{ fontSize: getStarSize() }}>
        ⚠️
      </div>
    );
  }
