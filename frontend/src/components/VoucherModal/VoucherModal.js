...
const VoucherModal = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const checkGoldenHourVouchers = async () => {
      ...
      const now = Date.now();
      if (now - lastGoldenHourCheck < 5 * 60 * 1000) {
  ...
  const getVoucherIcon = (code) => {
    if (code.startsWith("FIRST"))
      return <FaStar className="voucher-type-icon first" />;
    if (code.startsWith("LOYAL"))
      return <FaCrown className="voucher-type-icon loyal" />;
    if (code.startsWith("WELCOME"))
      return <FaGift className="voucher-type-icon welcome" />;
    if (code.startsWith("SW"))
      return <FaRegClock className="voucher-type-icon server" />;
        ...
        <div className="voucher-tabs">
          ...
          >
            Khả dụng (
            {
              vouchers.filter(
                (v) =>
                  v && new Date(v.ngayketthuc) >= new Date() && v.soluong > 0
              ).length
            }
            )
          </button>
        ...
        <div className="voucher-modal-content">
          {loading ? (
            ...
            <div className="no-vouchers">
              <p>
                {activeTab === "available"
                  ? "Bạn chưa có mã giảm giá khả dụng nào"
                  : "Bạn chưa có mã giảm giá hết hạn nào"}
              </p>
            </div>
          ) : (
            <div className="vouchers-list">
              {filteredVouchers.map((voucher, index) => {
                ...
                const isGoldenHour =
                  voucher.goldenHourStart && voucher.goldenHourEnd;
                const isCurrentlyGoldenHour =
                  isGoldenHour && isInGoldenHour(voucher);
                  ...
                  >
                    <div className="voucher-header">
                      ...
                      {isCurrentlyGoldenHour && (
                        <div className="active-now-label">Đang áp dụng</div>
                      )}
                    </div>
                    ...
                    <div className="voucher-body">
                      ...
                      {voucher.maxOrderValue && (
                        <p className="max-order">
                          Đơn tối đa:{" "}
                          {voucher.maxOrderValue.toLocaleString("vi-VN")}đ
                        </p>
                      )}
                      ...
                      {isGoldenHour && (
                        <p
                          className={`golden-hour ${
                            isCurrentlyGoldenHour ? "active" : ""
                          }`}
                        >
                          <FaRegClock /> Khung giờ vàng:{" "}
                          {voucher.goldenHourStart} - {voucher.goldenHourEnd}
                          {isCurrentlyGoldenHour && (
                            <span className="active-badge">Đang diễn ra</span>
                          )}
                        </p>
                      )}
