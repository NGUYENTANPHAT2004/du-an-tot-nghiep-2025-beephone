...
const CHAT_API_KEY =
  process.env.REACT_APP_CHAT_API_KEY || "beeshop_secret_chat_api_key_2025";
...
export const ChatAIProvider = ({ children }) => {
  ...
  const endChat = () => {
    ...
    if (socket && isConnected) {
      ...
      axios
        .post(
          `${apiBaseUrl}/api/chat/end/${sessionId}`,
          {},
          {
            headers: {
              "x-api-key": CHAT_API_KEY,
  ...
  const loadChatHistory = () => {
    ...
    if (socket && isConnected) {
      ...
      // Fallback to REST API
      axios
        .get(`${apiBaseUrl}/api/chat/history/${sessionId}`, {
          headers: {
            "x-api-key": CHAT_API_KEY,
          },
        })
